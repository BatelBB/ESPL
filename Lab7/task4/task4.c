...
void command(cmdLine* parsed, process ** process_list, int dFlag);
void handleParsed(cmdLine* parsed, process ** process_list, int dFlag);
...
void printHistory(char** history, int cur){
    int count = HISTLEN-1;
    if(cur < HISTLEN){
        count = cur;
    }
    ...
    for(int i = count; i >= 0 ; i--){
        printf("%d %s", cur-i, history[((cur)-i)%HISTLEN]);
    }
}
...
int main(int argc, char** argv){
    ...
    cmdLine* parsed = NULL;
    int dFlag = 0;
    int curHist = -1;    //looks at the last one in
    ...
    *process_list = NULL;
    ...
    while (1)
    {
        ...
        else if(strncmp(parsed->arguments[0], "history", 7) == 0){
            printHistory(history, curHist);
        }
        else if(strncmp(parsed->arguments[0], "!!", 2) == 0){
            parsed = parseCmdLines(history[curHist%HISTLEN]);
            handleParsed(parsed, process_list, dFlag);
        }
        else if(strncmp(parsed->arguments[0], "!", 1) == 0){
            int index = atoi(parsed->arguments[0]+1);

            if(index > curHist || index < 0){
                fprintf(stdout, "invalid index");
            }
            else{
                parsed = parseCmdLines(history[(curHist- (curHist-index)) % HISTLEN]);
                handleParsed(parsed, process_list, dFlag);
            }
        }
        else{
            curHist++;
            history[curHist%HISTLEN] = malloc(strlen(line) + 1);
            strcpy(history[curHist%HISTLEN], line);
            handleParsed(parsed, process_list, dFlag);
        }
...
void handleParsed(cmdLine* parsed, process ** process_list, int dFlag){
    if(parsed->next != NULL){
        myPipeLine(parsed, parsed->next);
    }
    else{
        command(parsed, process_list, dFlag);
    }
}
