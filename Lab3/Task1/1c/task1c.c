...
link *detect(link *virus_list)
{
    FILE *fp;
    fp = loadFromFile();
    char *buffer = (calloc(20000, sizeof(int)));;
...
link *fix(link *virus_list)
{
    ...
    fprintf(stdout, "Enter the starting byte location:\n");
    startingByte = fgetc(stdin);
    fprintf(stdout, "Enter the size of the virus signature:\n");
    virusSig = fgetc(stdin);
