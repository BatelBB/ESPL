...
char* unit_to_format(int unit, int isHex) {
    static char* formatshexa[] = {"%#hhx\n", "%#hx\n", "No such unit", "%#x\n"};
    static char* formatsdec[] = {"%#hhd\n", "%#hd\n", "No such unit", "%#d\n"};
    if(isHex)
        return formatshexa[unit-1];
    return formatsdec[unit-1];
...
void print_units(FILE* output, char* buffer, int count, int isHexa, int u_s) {
    char* end = buffer + u_s*count;
    while (buffer < end) {
        //print ints
        int var = *((int*)(buffer));
        fprintf(output, unit_to_format(u_s, isHexa), var);
        buffer += u_s;
    }
}
