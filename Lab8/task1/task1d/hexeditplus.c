...
void memoryDisplay(state *s) 
{
    ...
    print_units(stdout, s->mem_buf, u, s->display_mode, s->unit_size);        
...
{
    char input[100];
    printf("Enter source-address, target-location and length: \n");
    ...
    FILE* file = fopen(s->file_name, "a+b");
    if (file == NULL){
        fprintf(stderr, "cant open file\n");
        return;
    }
    
    if(target_location > *s->file_name){
        if(s->debug_mode)
            fprintf(stderr, "Can't copy name\n");
        fclose(file);
        return;
    }
    ...
    {
        //problem with source_address != 0
        fwrite((unsigned char*) source_address, s->unit_size, length, file);
    }
...
{
	char input[100];
    printf("Enter location and val in hexa: \n");
